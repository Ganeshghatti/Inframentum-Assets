...
const formSchema = z.object({
  ...
  panelWattage: z.string().refine((val) => parseFloat(val) > 0, {
    message: "Panel wattage must be positive",
  }),
  costPerPanel: z.string().refine((val) => parseFloat(val) > 0, {
    message: "Cost per panel must be positive",
  }),
  costPerInverter: z.string().refine((val) => parseFloat(val) > 0, {
    message: "Cost per inverter must be positive",
  }),
  invertersPerMw: z.string().refine((val) => parseFloat(val) > 0, {
    message: "Inverters per MW must be positive",
  }),
  salePriceOfPower: z.string().refine((val) => parseFloat(val) > 0, {
    message: "Sale price of power must be positive",
  }),
  installCost: z.string().refine((val) => parseFloat(val) > 0, {
    message: "Installation cost must be positive",
  }),
  omCost: z.string().refine((val) => parseFloat(val) >= 0, {
    message: "O&M cost must be non-negative",
  }),
  electricityTariff: z.string().refine((val) => parseFloat(val) > 0, {
    message: "Electricity tariff must be positive",
  }),
  annualConsumption: z.string().refine((val) => parseFloat(val) > 0, {
    message: "Annual consumption must be positive",
  }),
  projectLifetime: z.string().refine((val) => parseInt(val) > 0, {
    message: "Project lifetime must be positive",
  }),
  debtToEquity: z.string().refine((val) => parseFloat(val) >= 0, {
    message: "Debt-to-equity ratio must be non-negative",
  }),
  interestRate: z.string().refine((val) => parseFloat(val) >= 0, {
    message: "Interest rate must be non-negative",
  }),
  loanTenure: z.string().refine((val) => parseInt(val) > 0, {
    message: "Loan tenure must be positive",
  }),
  currencyConversionRate: z.string().refine((val) => parseFloat(val) > 0, {
    message: "Currency conversion rate must be positive",
  }),
  incentives: z.string().refine((val) => parseFloat(val) >= 0, {
...
const formFields = [
  ...
  {
    name: "panelWattage",
    label: "Wattage of Each Solar Panel (W)",
    placeholder: "550",
  },
  ...
  {
    name: "invertersPerMw",
    label: "Inverters Required per MW",
    placeholder: "4",
  },
  {
    ...
    <div className="">
      <div className="flex justify-between items-center">
        <Button
          ...
          className="h-14 w-14 m-auto flex flex-col items-center border-none"
        >
          <X />
